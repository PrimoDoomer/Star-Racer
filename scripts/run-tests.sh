...
echo "[run-tests] Tuning constants in sync (tuning.txt → tuning.rs / tuning.gd)…"
if ! python3 "$RepoRoot/scripts/gen_tuning.py" --check; then
    echo "[run-tests] Stale generated tuning — run: py scripts/gen_tuning.py" >&2
    exit 1
fi
