...
Write-Host "[run-tests] Tuning constants in sync (tuning.txt → tuning.rs / tuning.gd)…"
& py (Join-Path $RepoRoot 'scripts/gen_tuning.py') --check
if ($LASTEXITCODE -ne 0) { Write-Host "[run-tests] Stale generated tuning — run: py scripts/gen_tuning.py" -ForegroundColor Red; exit $LASTEXITCODE }
