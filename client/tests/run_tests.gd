...
func _run_all() -> void:
	...
	test_road_sampler_passes_through_nodes()
...
# --- TrackLoader road spline (client/scripts/track_loader.gd) --------------
# The Catmull-Rom sampler must pass through every control node and keep a
# straight line straight, matching the server (server/src/track.rs road_samples).

func test_road_sampler_passes_through_nodes() -> void:
	var prim := {
		"type": "road", "size": [24, 1, 0], "segments": 4,
		"nodes": [
			{"position": [0, 0, 0]},
			{"position": [0, 0, -50]},
			{"position": [0, 0, -100]},
	...
	var samples := TrackLoader._sample_road(prim)
	_check(samples.size() == 2 * 4 + 1, "road sampler emits segments*spans + 1 points")
	if samples.size() == 9:
		_check((samples[0]["pos"] as Vector3).distance_to(Vector3(0, 0, 0)) < 1e-4,
			"road sampler starts at the first node")
		_check((samples[4]["pos"] as Vector3).distance_to(Vector3(0, 0, -50)) < 1e-4,
			"road sampler passes through the middle node")
		_check((samples[8]["pos"] as Vector3).distance_to(Vector3(0, 0, -100)) < 1e-4,
			"road sampler ends at the last node")
	for s in samples:
		var p: Vector3 = s["pos"]
		_check(absf(p.x) < 1e-4 and absf(p.y) < 1e-4, "collinear road nodes stay straight")
