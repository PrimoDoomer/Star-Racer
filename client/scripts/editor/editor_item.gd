...
func supports_scale() -> bool:
	# A road's shape comes from its nodes, not a size box — width/thickness are
	# edited numerically, so the scale gizmo doesn't apply.
	return get_type() != "road"
...
func _build_visual() -> Node3D:
	...
	match get_type():
		...
		"arc":
			holder.add_child(_arc_mesh_instance())
		"road":
			holder.add_child(_road_mesh_instance())
		"decor":
			var model := String(data.get("model", ""))
			if model.begins_with("res://"):
				holder.add_child(_box_mesh_instance())  # placeholder bound for GLB props
			else:
				holder.add_child(TrackLoader.DecorBuilder.build(model, get_size(), _color()))
		_:
...
func _arc_mesh_instance() -> MeshInstance3D:
	var s := get_size()
	var seg := int(data.get("segments", TrackLoader.ARC_DEFAULT_SEGMENTS))
	var sweep := float(data.get("sweep_deg", TrackLoader.ARC_DEFAULT_SWEEP_DEG))
	var centers := TrackLoader._arc_centers(s.z, sweep, maxi(seg, 1))
	var mi := MeshInstance3D.new()
	mi.mesh = TrackLoader._build_arc_mesh(centers, s.x, s.y)
	var mat := _flat_material(_color())
	mat.cull_mode = BaseMaterial3D.CULL_DISABLED
	mi.material_override = mat
	return mi

func _road_mesh_instance() -> MeshInstance3D:
	var mi := MeshInstance3D.new()
	var samples := TrackLoader._sample_road_seg(data, TrackLoader._road_segments(data) * TrackLoader.ROAD_SURFACE_REFINE)
	if samples.size() < 2:
		return mi
	var walls: bool = data.get("walls", false) == true
	var wall_h: float = float(data.get("wall_height", TrackLoader.ROAD_DEFAULT_WALL_HEIGHT))
	mi.mesh = TrackLoader._build_road_mesh(samples, walls, wall_h)
	var mat := _flat_material(_color())
	mat.cull_mode = BaseMaterial3D.CULL_DISABLED
	mi.set_surface_override_material(0, mat)
	if walls and mi.mesh.get_surface_count() > 1:
		var wall_mat := _flat_material(TrackLoader.WALL_DEFAULT_COLOR)
...
func _default_color() -> Color:
	match get_type():
		...
		"arc": return TrackLoader.FLOOR_DEFAULT_COLOR
		"road": return TrackLoader.FLOOR_DEFAULT_COLOR
		"decor": return TrackLoader.DECOR_DEFAULT_COLOR
...
func _bbox() -> Dictionary:
	...
	if get_type() == "arc":
		...
		for c in centers:
			lo.x = minf(lo.x, c.x - hw); lo.y = minf(lo.y, c.y - hw)
			hi.x = maxf(hi.x, c.x + hw); hi.y = maxf(hi.y, c.y + hw)
		return {
			"center": Vector3(0.5 * (lo.x + hi.x), s.y * 0.5, 0.5 * (lo.y + hi.y)),
			"size": Vector3(maxf(hi.x - lo.x, 0.1), maxf(s.y, 0.1), maxf(hi.y - lo.y, 0.1)),
		}
	if get_type() == "road":
		var samples := TrackLoader._sample_road(data)
		if samples.is_empty():
		...
		var maxw := 0.0
		for sdic in samples:
			var pp: Vector3 = sdic["pos"]
			lo = lo.min(pp); hi = hi.max(pp)
			maxw = maxf(maxw, float(sdic["width"]))
		var pad := Vector3(maxw * 0.5, maxf(s.y, 0.5), maxw * 0.5)
		lo -= pad; hi += pad
