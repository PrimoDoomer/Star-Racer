...
static func to_json(track_def: Dictionary) -> String:
	# Integer fields must serialise without a fractional part: the server expects
	# laps_to_win as a u8 and a primitive's segments as a u32. A SpinBox or a float
	# loaded from JSON would otherwise write e.g. 3.0 / 10.0.
	var out := track_def.duplicate()
	if out.has("laps_to_win"):
		out["laps_to_win"] = int(round(float(out["laps_to_win"])))
	if out.get("primitives", null) is Array:
		var prims: Array = []
		for p in out["primitives"]:
			if p is Dictionary and p.has("segments"):
				var pc: Dictionary = p.duplicate()
				pc["segments"] = int(round(float(pc["segments"])))
				prims.append(pc)
			else:
				prims.append(p)
...
static func validate(td: Dictionary) -> String:
	if String(td.get("id", "")).strip_edges() == "":
		return TranslationServer.translate("ed_err_no_id")
	if int(td.get("laps_to_win", 0)) < 1:
		return TranslationServer.translate("ed_err_laps")
	if not (td.get("gates", null) is Array):
		return TranslationServer.translate("ed_err_gates")
	var has_start := false
	...
	for g in td["gates"]:
		...
		if r == "finish" or r == "start_finish":
			has_finish = true
	if not has_start:
		return TranslationServer.translate("ed_err_no_start")
	if not has_finish:
		return TranslationServer.translate("ed_err_no_finish")
	return ""
