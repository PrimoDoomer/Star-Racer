...
var _right_stick_armed := true  # right-stick list scroll: one move per flick (re-arms at centre)
var _nav_focused := false  # focus is in the top-right nav cluster (reached by L/R only)
var _nav_button_list: Array[Button] = []  # Settings / Editor / About / Quit, in order
...
# The top-right nav cluster (Settings / Editor / About / Quit) is kept FOCUS_CLICK:
# reachable by mouse, and by the L/R shoulder cycle (which grabs focus explicitly),
# but NOT by the stick or keyboard traversal — so a touchy stick can't drift onto
# Quit by accident. Keyboard access is intentionally dropped for now.
...
# Move the L/R home cursor to a "stop": the 3 tabs (switch the central panel) or the
# nav cluster (idx == tab count; leaves the central panel on the last opened tab).
...
# Enter the nav cluster: make its buttons focusable for the stick WHILE inside, and
# trap them (left/right blocked, up/down move within, ends clamped) so the stick can't
# escape — leave with L/R or cancel.
...
# Move keyboard/controller focus to the vertical neighbour (±1). Drives the right
# stick scrolling the Settings bindings list; the scroll's follow_focus keeps the
# focused row visible.
...
func _input(event: InputEvent) -> void:
	...
	if _rebinding_joy_action != "":
		...
		if event is InputEventJoypadButton or event is InputEventJoypadMotion:
			get_viewport().set_input_as_handled()
			return
	# Right stick scrolls lists directly (alongside the D-pad and keyboard arrows):
	# the lobby list on the home page, and the bindings list in Settings. One move per
	# flick (re-arms when the stick recentres). Gated to those contexts so it never
	# eats the in-race camera look (which also reads the right stick's Y axis).
	if event is InputEventJoypadMotion and (event as InputEventJoypadMotion).axis == JOY_AXIS_RIGHT_Y \
			and (_settings_overlay.visible or (_on_welcome_tabs_context() and self.join_view.visible and not _nav_focused)):
		var val := (event as InputEventJoypadMotion).axis_value
		if absf(val) < 0.35:
			_right_stick_armed = true
		elif _right_stick_armed:
			_right_stick_armed = false
			if _settings_overlay.visible:
				_move_focus_vertical(1 if val > 0.0 else -1)
			else:
				_move_lobby_selection(1 if val > 0.0 else -1)
		get_viewport().set_input_as_handled()
		return
	...
	if _on_welcome_tabs_context():
		if event is InputEventJoypadButton and (event as InputEventJoypadButton).pressed:
			...
			# L/R cycle the home "stops": the 3 tabs, then the top-right nav cluster.
			# Landing on the nav cluster keeps the central panel on the last opened tab.
			...
			if not _nav_focused and self.join_view.visible and (b == JOY_BUTTON_DPAD_UP or b == JOY_BUTTON_DPAD_DOWN):
				...
				return
		var foc := get_viewport().gui_get_focus_owner()
		# Gamepad only: the tabs aren't stick-focusable, so accept on the Join tab dives
		# into the lobby list (focus-trap). On keyboard the list is FOCUS_ALL and sits in
		# the normal focus flow, so the arrow keys move into it and scroll rows directly —
		# no dive-in step.
...
func switch_mode(next_mode: Game.Mode, server_up: bool):
	...
	if next_mode == Game.Mode.WELCOME_PAGE:
		...
		_exit_nav_zone()             # …and not parked in the nav cluster
