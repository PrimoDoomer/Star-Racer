...
@onready var nickname_field: LineEdit = $OnlineMenu/Center/Container/CoursesPanel/CoursesBox/CarView/NicknameField
...
@onready var car_tab_button: Button = $OnlineMenu/Center/Container/CoursesPanel/CoursesBox/Tabs/CarTabButton
...
@onready var car_view: VBoxContainer = $OnlineMenu/Center/Container/CoursesPanel/CoursesBox/CarView
@onready var preview_slot: PanelContainer = $OnlineMenu/Center/Container/CoursesPanel/CoursesBox/CarView/PreviewSlot
...
@onready var _car_model_label: Label = $OnlineMenu/Center/Container/CoursesPanel/CoursesBox/CarView/ModelNav/ModelLabel
...
@onready var _nav_buttons_box: VBoxContainer = $OnlineMenu/NavButtons
@onready var _settings_button: Button = $OnlineMenu/NavButtons/SettingsButton
@onready var _about_button: Button = $OnlineMenu/NavButtons/AboutButton
...
# Horn picker (Pilot tab): selected index, the name label, a header, and a player
# that auditions the horn when you change selection.
var _horn_idx: int = 0
var _horn_header: Label = null
var _horn_model_label: Label = null
var _horn_preview: AudioStreamPlayer = null
...
var _sound_master_row := {}  # {"label": Label, "slider": HSlider, "pct": Label}
var _sound_sfx_row := {}
var _sound_horn_row := {}
var _settings_graphics_label: Label = null
var _perf_check: CheckBox = null
var _settings_controls_label: Label = null
var _settings_scroll_hint: Label = null
...
var _rs_repeat := 0.0
var _nav_focused := false
var _nav_button_list: Array[Button] = []

# Lobby list filtering: keep the last fetched list so the "hide full sessions"
# toggle can re-render without a server round-trip.
var _all_lobby_infos: Array = []
var _hide_full := false
var _hide_full_check: CheckBox = null
# Per-lobby gameplay-tweak panel (built in code) + the button that opens it, parked
# at the bottom of the Create view. get_gameplay_tweaks() reads the panel's fields.
const TweakPanel = preload("res://scripts/tweak_panel.gd")
var _tweak_panel: Control = null
var _tweak_button: Button = null

# Pad-only hints: ▲▼ "browse the list" under the lobby list, and L / R key-caps on
# the tabs (shoulder buttons switch tabs). Plus the quit confirmation. Built in code.
var _pad_browse_hint: Control = null  # the ▲▼ + label row (toggled in _process)
var _pad_browse_label: Label = null
var _tab_hint_l: Label = null
var _tab_hint_r: Label = null
...
# Last status shown as a locale KEY (+ format args), so the bottom-right info pill
# re-translates live when the language changes. "" = the pill holds raw text that
# can't be re-translated (transient race messages, which refresh on their own).
...
func _ready() -> void:
	...
	_build_about_panel()
	_build_quit_confirm()
	_setup_tab_style()       # real "tab" look on every platform (was mobile-only)
	_setup_nav_buttons()
	_build_pad_browse_hint()
	_build_hide_full_check()
	_build_tweak_panel()
	_build_tab_pad_hints()
	_style_info_label()
...
func _update_car_preview() -> void:
	...
	if _car_model_label != null:
		_car_model_label.text = tr(model_def["name"])
...
# Horn picker, built in code and slotted into the Pilot tab between the car nav and
# the nickname. A small header disambiguates it from the car ‹ name › row; changing
# selection auditions the horn through a dedicated preview player.
func _build_horn_selector() -> void:
	var box := VBoxContainer.new()
	box.add_theme_constant_override("separation", 6)

	_horn_header = Label.new()
	_horn_header.text = tr("ui_horn")
	_horn_header.add_theme_font_size_override("font_size", 13)
	_horn_header.add_theme_color_override("font_color", ACCENT)
	box.add_child(_horn_header)
	...
	prev.pressed.connect(_on_horn_prev)
	nav.add_child(prev)
	_horn_model_label = Label.new()
	_horn_model_label.size_flags_horizontal = SIZE_EXPAND_FILL
	_horn_model_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_horn_model_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	nav.add_child(_horn_model_label)
	var nxt := Button.new()
	nxt.text = "›"
	nxt.custom_minimum_size = Vector2(40, 0)
	nxt.pressed.connect(_on_horn_next)
	nav.add_child(nxt)
	box.add_child(nav)

	self.car_view.add_child(box)
	self.car_view.move_child(box, 2)  # after the car ModelNav, before the nickname

	_horn_preview = AudioStreamPlayer.new()
	_horn_preview.volume_db = -3.0
	_horn_preview.bus = Audio.BUS_HORN
	add_child(_horn_preview)
	_update_horn_label()

func _update_horn_label() -> void:
	if _horn_model_label and _horn_idx < Game.HORNS.size():
		_horn_model_label.text = tr(Game.HORNS[_horn_idx]["name"])

func _play_horn_preview() -> void:
	if _horn_preview:
		_horn_preview.stream = load(Game.HORNS[_horn_idx]["path"])
		_horn_preview.play()

func _on_horn_prev() -> void:
	_horn_idx = (_horn_idx - 1 + Game.HORNS.size()) % Game.HORNS.size()
	_update_horn_label()
	_play_horn_preview()

func _on_horn_next() -> void:
	_horn_idx = (_horn_idx + 1) % Game.HORNS.size()
	_update_horn_label()
	_play_horn_preview()

func get_horn_id() -> String:
	return Game.HORNS[_horn_idx]["id"]

func set_horn_id(horn_id: String) -> void:
	for i in Game.HORNS.size():
		if Game.HORNS[i]["id"] == horn_id:
			_horn_idx = i
			break
	_update_horn_label()
...
# The home page is a row of tabs (Join | Create | Car), each toggling its view.
# A single ordered list keeps the radio behaviour, L/R cycling and pad-focus rules
# generic instead of hard-coding two tabs.
enum Tab { JOIN, CREATE, CAR }

func _tabs() -> Array:
	return [
		[self.join_tab_button, self.join_view],
		[self.create_tab_button, self.create_view],
		[self.car_tab_button, self.car_view],
	]

# Index of the currently active tab (defaults to Join).
func _current_tab() -> int:
	if self.create_tab_button.button_pressed:
		return Tab.CREATE
	if self.car_tab_button.button_pressed:
		return Tab.CAR
	return Tab.JOIN

func _on_join_tab_toggled(on: bool) -> void:
	_on_tab_toggled(Tab.JOIN, on)

func _on_create_tab_toggled(on: bool) -> void:
	_on_tab_toggled(Tab.CREATE, on)

func _on_car_tab_toggled(on: bool) -> void:
	_on_tab_toggled(Tab.CAR, on)

# Radio behaviour: turning a tab on selects it; toggling the active tab off snaps it
# back on (you can't have zero tabs). Doesn't steal focus — the click already set it.
func _on_tab_toggled(idx: int, on: bool) -> void:
	if on:
		_select_tab_index(idx, false)
	else:
		(_tabs()[idx][0] as Button).set_pressed_no_signal(true)

# Switch the active home tab. `grab` moves focus into the new tab (used by the L/R
# shoulder shortcut); leave it false for a mouse click that already moved focus.
func _select_tab_index(idx: int, grab: bool = true) -> void:
	_release_lobby_list_focus()  # bailing to another tab also leaves the list
	var tabs := _tabs()
	for i in tabs.size():
		(tabs[i][0] as Button).set_pressed_no_signal(i == idx)
		(tabs[i][1] as Control).visible = i == idx
	if grab:
		# With a pad the tabs aren't stick-focusable (L/R switch them), so drop focus
		# INTO the new view; on keyboard the tab button itself takes focus.
		if _using_gamepad:
			_focus_active_view()
		else:
			(tabs[idx][0] as Button).grab_focus()

# Land focus on a visible, enabled widget inside the active tab's view. The tabs
# aren't stick-focusable under a controller and the lobby list is browsed with the
# D-pad (not focus), so the Join view lands on Refresh — giving the stick a foothold
# to reach every button. Used on entering the page, switching tabs, and recovering
# focus that a now-unfocusable control dropped.
func _focus_active_view() -> void:
	var candidates: Array[Control] = []
	if self.create_view.visible:
		candidates = [self.lobby_name_field, self.create_button]
	elif self.car_view.visible:
		candidates = [self.nickname_field]
	else:  # join view
		candidates = [self.refresh_list_button, _hide_full_check, self.join_button]
	for c in candidates:
		if c != null and c.visible and not (c is Button and (c as Button).disabled):
			c.grab_focus()
			return

func _setup_nav_buttons() -> void:
	for child in _nav_buttons_box.get_children():
		if child is Button:
			_nav_button_list.append(child)
			(child as Button).focus_mode = Control.FOCUS_CLICK

func _go_to_home_stop(idx: int) -> void:
	if idx < _tabs().size():
		_exit_nav_zone()
		_select_tab_index(idx)
	else:
		_enter_nav_zone()

func _enter_nav_zone() -> void:
	if _nav_focused or _nav_button_list.is_empty():
		return
	_nav_focused = true
	for i in _nav_button_list.size():
		var btn := _nav_button_list[i]
		btn.focus_mode = Control.FOCUS_ALL
		btn.focus_neighbor_left = btn.get_path()
		btn.focus_neighbor_right = btn.get_path()
		btn.focus_neighbor_top = (_nav_button_list[i - 1] if i > 0 else btn).get_path()
		btn.focus_neighbor_bottom = (_nav_button_list[i + 1] if i < _nav_button_list.size() - 1 else btn).get_path()
	_nav_button_list[0].grab_focus()

func _exit_nav_zone() -> void:
	if not _nav_focused:
		return
	_nav_focused = false
	for btn in _nav_button_list:
...
# Move the lobby-list selection by `dir` (±1) across selectable rows and scroll it
# into view, without the list needing keyboard focus. Driven by the D-pad on a
# controller (see _input), so browsing races is decoupled from form navigation.
func _move_lobby_selection(dir: int) -> void:
	var t := self.lobbies_list
	var root := t.get_root()
	if root == null:
		return
	...
	if rows.is_empty():
		return
	var idx := rows.find(t.get_selected())
	idx = 0 if idx == -1 else clampi(idx + dir, 0, rows.size() - 1)
	rows[idx].select(0)
	t.scroll_to_item(rows[idx])

func _move_focus_vertical(dir: int) -> void:
	var foc := get_viewport().gui_get_focus_owner()
	if foc == null:
		return
	var nb := foc.find_valid_focus_neighbor(SIDE_BOTTOM if dir > 0 else SIDE_TOP)
	if nb:
		nb.grab_focus()

func _scroll_lists_with_right_stick(delta: float) -> void:
	var in_list := _using_gamepad and (_settings_overlay.visible \
		or (_on_welcome_tabs_context() and self.join_view.visible and not _nav_focused))
	if not in_list:
		_rs_repeat = 0.0
		return
	var rs := Input.get_action_strength("View Down") - Input.get_action_strength("View Up")
	if absf(rs) < 0.2:
		_rs_repeat = 0.0
		return
	_rs_repeat -= delta
	if _rs_repeat > 0.0:
		return
	_rs_repeat = lerpf(0.30, 0.05, clampf((absf(rs) - 0.2) / 0.8, 0.0, 1.0))
	var dir := 1 if rs > 0.0 else -1
	if _settings_overlay.visible:
		_move_focus_vertical(dir)
	else:
		_move_lobby_selection(dir)

# Controller navigation model on the home page (desktop): the left stick navigates
# the form, the D-pad scrolls the lobby list, and L/R switch tabs — so the Join/
# Create tabs and the lobby list are taken OUT of stick focus (FOCUS_CLICK) while a
# pad is in use, and restored for keyboard/mouse. Called each frame from _process.
func _update_pad_focus_modes() -> void:
	if Game.is_mobile():
		return  # mobile already pins the list to FOCUS_CLICK; no tabs/pad nav
	var want := Control.FOCUS_CLICK if _using_gamepad else Control.FOCUS_ALL
	for b: Button in [self.join_tab_button, self.create_tab_button, self.car_tab_button]:
		if b.focus_mode != want:
			b.focus_mode = want
	# The keyboard "dive into the list" focus-trap owns the list's focus_mode while
	# active; don't fight it. Otherwise mirror the tabs.
	if not _in_lobby_list and self.lobbies_list.focus_mode != want:
		self.lobbies_list.focus_mode = want
	# A control that just lost focusability (pad picked up while it held focus) would
	# strand the cursor — pull focus back to the form.
	var foc := get_viewport().gui_get_focus_owner()
	if _using_gamepad and (foc == self.join_tab_button or foc == self.create_tab_button \
			or foc == self.car_tab_button \
			or (foc == self.lobbies_list and not _in_lobby_list)):
		_focus_active_view()
...
# ----------------------------------------------------------------------------
# About panel: version/build/source, a "Made by" section (designer + Claude, two
# equal credits) and third-party credits (CC0 first). Same overlay pattern as
# the settings panel, built in code.
...
func _build_about_panel() -> void:
	...
	# Made by — the designer and Claude as two SEPARATE, EQUAL credits (same weight,
	# no maker's-mark hierarchy): two people, side by side.
	box.add_child(_make_about_label("about_team", ACCENT, 13))
	box.add_child(_make_about_label("about_team_designer", Color(0.88, 0.92, 0.98), 0))
	box.add_child(_make_about_label("about_team_claude", Color(0.88, 0.92, 0.98), 0))

	box.add_child(HSeparator.new())

	# Credits — third-party assets & tech. CC0 first and clearly labelled: public-domain
	# assets are always credited with thanks even though attribution isn't required.
	box.add_child(_make_about_label("about_credits", ACCENT, 13))
	box.add_child(_make_about_label("about_credits_cc0", Color(0.88, 0.92, 0.98), 0))
...
# Browse hint: hand-DRAWN up/down triangles + a label. Drawn (not a glyph) so it
# renders identically everywhere, including the web export where the bundled font
# lacks ▲▼ and SystemFont isn't available (they showed as tofu in the browser).
func _build_pad_browse_hint() -> void:
	...
	var arrows := Control.new()
	arrows.custom_minimum_size = Vector2(13, 22)
	arrows.mouse_filter = Control.MOUSE_FILTER_IGNORE
	arrows.draw.connect(_draw_updown_arrows.bind(arrows))
	row.add_child(arrows)

	_pad_browse_label = Label.new()
	_pad_browse_label.add_theme_font_size_override("font_size", 13)
	_pad_browse_label.add_theme_color_override("font_color", ACCENT_BRIGHT)
	_pad_browse_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	row.add_child(_pad_browse_label)

	self.join_view.add_child(row)
	self.join_view.move_child(row, 1)  # between the list and the footer
	_pad_browse_hint = row
	_refresh_pad_browse_hint()

func _draw_updown_arrows(c: Control) -> void:
	var w := c.size.x
	var h := c.size.y
	var cx := w * 0.5
	var half := w * 0.5
	var up := PackedVector2Array([Vector2(cx, 0.0), Vector2(cx - half, h * 0.42), Vector2(cx + half, h * 0.42)])
	var dn := PackedVector2Array([Vector2(cx, h), Vector2(cx - half, h * 0.58), Vector2(cx + half, h * 0.58)])
	c.draw_colored_polygon(up, ACCENT_BRIGHT)
	c.draw_colored_polygon(dn, ACCENT_BRIGHT)

func _refresh_pad_browse_hint() -> void:
	if _pad_browse_label:
		_pad_browse_label.text = tr("pad_browse")

# "Hide full sessions" toggle, sitting just above the Join/Refresh footer. Filters
# full lobbies out of the list (re-rendered from the cached fetch, no round-trip).
func _build_hide_full_check() -> void:
	_hide_full_check = CheckBox.new()
	_hide_full_check.text = tr("ui_hide_full")
	_hide_full_check.button_pressed = _hide_full
	_hide_full_check.add_theme_font_size_override("font_size", 14)
	_hide_full_check.toggled.connect(_on_hide_full_toggled)
	self.join_view.add_child(_hide_full_check)
	# Park it directly above the footer (Refresh / Join), at the bottom of the view.
	self.join_view.move_child(_hide_full_check, self.refresh_list_button.get_parent().get_index())

func _build_tweak_panel() -> void:
	_tweak_panel = TweakPanel.new()
	add_child(_tweak_panel)
	_tweak_button = Button.new()
	_tweak_button.text = tr("tweak_gameplay")
	_tweak_button.add_theme_font_size_override("font_size", 14)
	_tweak_button.pressed.connect(func(): _tweak_panel.open())
	self.create_view.add_child(_tweak_button)
	# Park it directly above the Create button at the bottom of the Create view.
	self.create_view.move_child(_tweak_button, self.create_button.get_index())

# The creator's chosen overrides (sparse: only changed constants). Sent in the
# CreateLobby request; the server echoes them to every joiner for lockstep.
func get_gameplay_tweaks() -> Dictionary:
	return _tweak_panel.get_tweaks() if _tweak_panel else {}

func _on_hide_full_toggled(on: bool) -> void:
	_hide_full = on
	_render_lobbies()

# L / R key-caps flanking the Join | Create tabs, so it reads that the shoulder
# buttons switch tabs. Plain ASCII (renders in any font) on a rounded key-cap.
# Shown only with a controller in hand (see _process).
func _build_tab_pad_hints() -> void:
	var tabs := self.join_tab_button.get_parent() as HBoxContainer
	_tab_hint_l = _make_pad_key_label("L")
	tabs.add_child(_tab_hint_l)
	tabs.move_child(_tab_hint_l, 0)  # left of the Join tab
	_tab_hint_r = _make_pad_key_label("R")
	tabs.add_child(_tab_hint_r)  # right of the Create tab (appended last)

func _make_pad_key_label(txt: String) -> Label:
	var l := Label.new()
	l.text = txt
	...
	var sb := StyleBoxFlat.new()
	sb.bg_color = Color(0.20, 0.23, 0.29)
	sb.set_corner_radius_all(6)
...
# Show a status from a locale key (+ optional `%` args), remembering it so the pill
# re-translates when the language changes (see _apply_locale). Use this for any
# message that lingers on screen; transient per-tick messages can use _set_status.
func _set_status_tr(key: String, kind: int = StatusKind.INFO, args: Array = []) -> void:
	_status_key = key
	_status_args = args
	var text := tr(key)
	...
	# _set_status clears the key for the raw-text path; restore it so we re-translate.
	_status_key = key
	_status_args = args

func _set_status(text: String, kind: int = StatusKind.INFO) -> void:
	_status_kind = kind
	_status_key = ""  # raw text by default; the _tr variant re-sets this
	_status_args = []
	if text.strip_edges() == "":
...
func _apply_locale() -> void:
	...
	if _tweak_button:
		_tweak_button.text = tr("tweak_gameplay")
	if _tweak_panel:
		_tweak_panel.relocalize()
	_refresh_settings_labels()
	_refresh_binding_rows()
	if _car_model_label != null and _car_model_idx < Game.CAR_MODELS.size():
		_car_model_label.text = tr(Game.CAR_MODELS[_car_model_idx]["name"])
	if _horn_header != null:
		_horn_header.text = tr("ui_horn")
	_update_horn_label()
	# Re-translate the bottom-right info pill if it's showing a keyed message.
	if _status_key != "" and self.info_label.visible:
		_set_status_tr(_status_key, _status_kind, _status_args)
...
func _build_settings_panel() -> void:
	...
	# Sound section: master / effects / horn volume sliders (Audio autoload buses).
	_settings_sound_label = _make_section_label()
	box.add_child(_settings_sound_label)
	_sound_master_row = _make_volume_row(box, Audio.master(), Audio.set_master)
	_sound_sfx_row = _make_volume_row(box, Audio.sfx(), Audio.set_sfx)
	_sound_horn_row = _make_volume_row(box, Audio.horn(), Audio.set_horn)

	box.add_child(HSeparator.new())

	# Graphics section: a single performance-mode toggle (low quality, higher FPS).
	_settings_graphics_label = _make_section_label()
	box.add_child(_settings_graphics_label)
	_perf_check = CheckBox.new()
	_perf_check.button_pressed = Graphics.is_low()
	_perf_check.toggled.connect(_on_perf_toggled)
	box.add_child(_perf_check)
	...
	_settings_scroll_hint = Label.new()
	_settings_scroll_hint.add_theme_font_size_override("font_size", 12)
	_settings_scroll_hint.add_theme_color_override("font_color", ACCENT_BRIGHT)
	_settings_scroll_hint.visible = false
	box.add_child(_settings_scroll_hint)
...
# A volume row: name label (left) + 0..1 slider + a live "NN%" readout. `apply` is
# the Audio setter to push the value to; returns the row's parts for re-translation.
func _make_volume_row(parent: Control, value: float, apply: Callable) -> Dictionary:
	var row := HBoxContainer.new()
	row.size_flags_horizontal = SIZE_EXPAND_FILL
	row.add_theme_constant_override("separation", 12)
	...
	slider.value_changed.connect(func(v: float) -> void:
		...
		apply.call(v))
	parent.add_child(row)
	return {"label": name_label, "slider": slider, "pct": pct}
...
func _refresh_settings_labels() -> void:
	...
	if _settings_sound_label != null:
		_settings_sound_label.text = tr("set_sound")
		_sound_master_row["label"].text = tr("set_vol_master")
		_sound_sfx_row["label"].text = tr("set_vol_sfx")
		_sound_horn_row["label"].text = tr("set_vol_horn")
	if _settings_graphics_label != null:
		_settings_graphics_label.text = tr("set_graphics")
	if _perf_check != null:
		_perf_check.text = tr("set_perf_mode")
	_settings_controls_label.text = tr("set_controls")
	if _settings_scroll_hint:
		_settings_scroll_hint.text = tr("pad_scroll_list")
...
func _input(event: InputEvent) -> void:
	...
	if _rebinding_joy_action != "":
		...
		if event is InputEventJoypadButton or event is InputEventJoypadMotion:
			get_viewport().set_input_as_handled()
			return
	# Home tabs: shoulder L/R cycle Join | Create | Car; the action key on the Join
	# tab dives into the lobby list; back returns from the list to the tabs.
	if _on_welcome_tabs_context():
		if event is InputEventJoypadButton and (event as InputEventJoypadButton).pressed:
			...
			if b == JOY_BUTTON_LEFT_SHOULDER or b == JOY_BUTTON_RIGHT_SHOULDER:
				get_viewport().set_input_as_handled()
				var stops := _tabs().size() + 1
				var cur := stops - 1 if _nav_focused else _current_tab()
				var step := 1 if b == JOY_BUTTON_RIGHT_SHOULDER else stops - 1
				_go_to_home_stop((cur + step) % stops)
				return
			# The D-pad is reserved for the lobby list: up/down move the selection
			# directly (the left stick handles general form navigation), so browsing
			# races never fights with reaching the form fields. Join view only.
			if not _nav_focused and self.join_view.visible and (b == JOY_BUTTON_DPAD_UP or b == JOY_BUTTON_DPAD_DOWN):
				get_viewport().set_input_as_handled()
				_move_lobby_selection(-1 if b == JOY_BUTTON_DPAD_UP else 1)
				return
		var foc := get_viewport().gui_get_focus_owner()
		if _using_gamepad and event.is_action_pressed("ui_accept") and foc == self.join_tab_button and self.join_view.visible:
			get_viewport().set_input_as_handled()
			_enter_lobby_list()
			return
		...
		if foc == self.lobbies_list and event.is_action_pressed("ui_accept") and not self.join_button.disabled:
			...
			return
		if _nav_focused and event.is_action_pressed("ui_cancel"):
			get_viewport().set_input_as_handled()
			_exit_nav_zone()
			_focus_active_view()
			return
...
func _process(delta: float) -> void:
	_scroll_lists_with_right_stick(delta)
	# A row is selectable even when full (you can highlight it), but a full lobby isn't
	# joinable — so disable Join when nothing is selected OR the selection is full.
	var sel_item := self.lobbies_list.get_selected()
	self.join_button.disabled = sel_item == null or bool(sel_item.get_metadata(0))
	# Controller-only hints: L/R key-caps on the tabs (both tabs), and the ▲▼
	# list-browse hint (Join tab only). Shown only with a controller in hand and no
	# overlay up; they vanish the moment the player touches keyboard/mouse.
	var pad_ctx: bool = _using_gamepad and self.online_menu.visible \
		and not _settings_overlay.visible and not _about_overlay.visible \
		and not _quit_overlay.visible
	# Keep the tabs / lobby list in or out of stick-focus navigation per input device.
	if self.online_menu.visible:
		_update_pad_focus_modes()
	if _tab_hint_l:
		_tab_hint_l.visible = pad_ctx
	if _tab_hint_r:
		_tab_hint_r.visible = pad_ctx
	if _pad_browse_hint:
		_pad_browse_hint.visible = pad_ctx and self.join_view.visible
	if _settings_scroll_hint:
		_settings_scroll_hint.visible = _settings_overlay.visible and _using_gamepad
...
func switch_mode(next_mode: Game.Mode, server_up: bool):
	if %Game.mode == Game.Mode.WELCOME_PAGE:
		...
		if next_mode == Game.Mode.FETCH_LOBBIES:
			_set_status_tr("fetching", StatusKind.LOADING)
	...
	if next_mode == Game.Mode.WELCOME_PAGE:
		...
		if self.game_node.mode == Game.Mode.IN_RACE \
		   ...
		   || self.game_node.mode == Game.Mode.SPECTATOR:
			...
			if !server_up:
				_set_status_tr("no_connection", StatusKind.ERROR)
				self.join_button.disabled = true
				self.create_button.disabled = true
			else:
				_set_status_tr("lobbies_fetched", StatusKind.INFO)
			...
			_select_tab_index(Tab.JOIN, false)
			self.refresh_list_button.grab_focus()
		# Make sure a widget holds focus so a controller can navigate. With a pad the
		# tabs are out of stick-focus (L/R switch them), so land inside the active view
		# (its widgets are visible); on keyboard the tab itself is fine.
		...
		self.intermission_menu.visible = false
		self.menu_background.visible = false
		_set_status_tr("spectating", StatusKind.INFO)
...
func refresh(lobby_infos: Array):
	# Keep the raw list so the "hide full" toggle can re-render without re-fetching.
	_all_lobby_infos = lobby_infos.duplicate()
	_render_lobbies()
...
func _render_lobbies() -> void:
	...
	# Optionally drop full sessions (the bottom-of-list checkbox), then order the rest:
	# open races you can jump into first, then races in progress, then full ones — so
	# the most useful rows are right at the top of the list.
	var infos := _all_lobby_infos.duplicate()
	if _hide_full:
		infos = infos.filter(func(i): return int(i.player_count) < int(i.max_players))
	...
	if infos.is_empty():
		...
		if _status_kind != StatusKind.ERROR:
			_set_status_tr("no_lobbies", StatusKind.INFO)
		return
	...
	for info in infos:
		...
		# Remember full-ness on the row so the Join button can stay disabled while a
		# full lobby is selected (a full row is still selectable — see _process).
		...
		# A full lobby stays selectable (you can highlight it to read its details), but
		# isn't joinable — the Join button is disabled while a full row is selected
		# (see _process). Dim it so "full" still reads at a glance.
		if full:
			for c in [0, 1, 6]:
...
func show_race_results(rankings: Array) -> void:
	...
	for i in rankings.size():
		self.players_in_lobby.add_child(_make_result_row(i + 1, String(rankings[i])))
	_set_status_tr("race_finished", StatusKind.SUCCESS)

# A finish-order row: a medal-coloured position badge + the racer's name, with the
# top three tinted gold/silver/bronze and the winner enlarged and highlighted.
func _make_result_row(rank: int, racer_name: String) -> PanelContainer:
	var medal := ROW_DIM
	if rank == 1:
		medal = Color(1.0, 0.84, 0.20)    # gold
	elif rank == 2:
		medal = Color(0.80, 0.84, 0.88)   # silver
	elif rank == 3:
		medal = Color(0.82, 0.52, 0.28)   # bronze
	var winner := rank == 1

	var chip := PanelContainer.new()
	chip.size_flags_horizontal = SIZE_FILL
	var sb := StyleBoxFlat.new()
	sb.bg_color = Color(0.12, 0.14, 0.18, 0.97) if winner else Color(0.09, 0.105, 0.13, 0.94)
	sb.set_corner_radius_all(8)
	sb.border_width_left = 6 if winner else 4
	sb.border_color = medal
	sb.content_margin_left = 14.0
	sb.content_margin_right = 16.0
	sb.content_margin_top = 12.0 if winner else 9.0
	sb.content_margin_bottom = 12.0 if winner else 9.0
	chip.add_theme_stylebox_override("panel", sb)
	...
	# Position badge: the rank number on a rounded medal-coloured square.
	var badge := Label.new()
	badge.text = str(rank)
	badge.custom_minimum_size = Vector2(34, 34)
	badge.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	badge.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	badge.add_theme_font_size_override("font_size", 18 if winner else 16)
	badge.add_theme_color_override("font_color", Color(0.06, 0.07, 0.09))
	var bsb := StyleBoxFlat.new()
	bsb.bg_color = medal
	...
	nm.add_theme_font_size_override("font_size", 22 if winner else 17)
	nm.add_theme_color_override("font_color", Color(1.0, 0.95, 0.80) if winner else Color(0.90, 0.93, 0.97))
	row.add_child(nm)
