...
func _draw() -> void:
	...
	# Discreet ring captions in the open bottom gap, colour-matched so each names its
	# ring: TURBO (inner, blue) sits higher, BOOST (outer, amber) just below it.
...
# A tiny centred caption sitting `radius` below the gauge centre (in the open bottom
# gap), used to name each ring discreetly.
