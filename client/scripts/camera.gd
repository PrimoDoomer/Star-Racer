...
# While drifting, swing the framing this far (0..1) from the chassis heading toward
# the car's actual travel heading, so the slide — and steering into it — reads on
# screen as the camera leading the drift.
const DRIFT_LOOK_BIAS := 0.6

# Position + look follow are critically-damped springs (SmoothDamp): they ease in
# AND out with velocity continuity, so a sudden car move (drift snap, boost kick)
# is cushioned instead of yanking the camera. `smooth_time` ≈ the time to settle;
# the drift value is a touch longer so the frame floats more while sliding.
const POS_SMOOTH_TIME       := 0.13
const POS_SMOOTH_TIME_DRIFT := 0.20
const LOOK_SMOOTH_TIME      := 0.10
...
const FOV_DRIFT_ADD := 4.0    # extra degrees while drifting, so a slide reads as energy
const FOV_ATTACK_RATE := 8.0  # ease-in rate (widening — snappy)
const FOV_RELEASE_RATE := 3.0 # ease-out rate (narrowing — smooth)

var _cam_pos     := Vector3.ZERO
var _cam_vel     := Vector3.ZERO  # SmoothDamp velocity state for the position spring
var _look_target := Vector3.ZERO
var _look_vel    := Vector3.ZERO  # SmoothDamp velocity state for the look-target spring
var _cam_yaw    := 0.0
var _orb_yaw    := 0.0
var _orb_pitch  := 0.0
...
# Framerate-independent exponential approach factor: lerp by this toward a target
# decays the remaining distance at `rate` per second regardless of frame time.
static func _approach(rate: float, delta: float) -> float:
	return 1.0 - exp(-rate * delta)

# Critically-damped spring step (Unity-style SmoothDamp). Eases toward `target`
# with velocity continuity (no instant jumps), settling in ~`smooth_time` seconds.
# Returns [new_value, new_velocity]; pass the stored velocity back in each frame.
static func _smooth_damp(current: Vector3, target: Vector3, vel: Vector3, smooth_time: float, delta: float) -> Array:
...
func _snap_to_car(yaw_offset: float = 0.0) -> void:
	...
	self._cam_vel     = Vector3.ZERO
	self._look_vel    = Vector3.ZERO
...
func _tick_orbital(delta: float) -> void:
	...
	if abs(sx) > 0.0:
		...
		self._orb_yaw = lerpf(self._orb_yaw, 0.0, _approach(ORBIT_RETURN, delta))

	if abs(sy) > 0.0:
		self._orb_pitch = clamp(self._orb_pitch - sy * ORBIT_SENS * delta, -ORBIT_PITCH_MAX, ORBIT_PITCH_MAX)
	else:
		self._orb_pitch = lerpf(self._orb_pitch, 0.0, _approach(ORBIT_RETURN, delta))
...
func _tick_yaw(delta: float) -> void:
	...
	self._drift_blend = lerpf(self._drift_blend, 1.0 if drifting else 0.0, _approach(DRIFT_CAM_BLEND_RATE, delta))

	# Base framing is behind the chassis; while drifting, swing it toward the car's
	# actual travel heading so the slide and steering read on screen. vel_yaw uses
	# the same convention as _cam_yaw (camera sits behind, so negate the velocity).
	var heading := self._car.global_rotation.y
	var vel := self._car.linear_velocity
	var vel_speed := Vector2(vel.x, vel.z).length()
	if self._drift_blend > 0.01 and vel_speed > 3.0:
		var vel_yaw := atan2(-vel.x, -vel.z)
		heading = lerp_angle(heading, vel_yaw, DRIFT_LOOK_BIAS * self._drift_blend)
	var target_yaw := heading + self._orb_yaw

	var rate := lerpf(YAW_RATE_GRIP, YAW_RATE_DRIFT, self._drift_blend)
	var diff := angle_difference(self._cam_yaw, target_yaw)
	self._cam_yaw += diff * _approach(rate, delta)
...
func _tick_position(delta: float) -> void:
	...
	# Cushioned spring follow; floats a touch more while drifting (longer settle).
	var pos_time := lerpf(POS_SMOOTH_TIME, POS_SMOOTH_TIME_DRIFT, self._drift_blend)
	var pr := _smooth_damp(self._cam_pos, target, self._cam_vel, pos_time, delta)
	self._cam_pos = pr[0]
	self._cam_vel = pr[1]

	var look_goal := self._car.global_position + Vector3(0.0, LOOK_HEIGHT, 0.0)
	var lr := _smooth_damp(self._look_target, look_goal, self._look_vel, LOOK_SMOOTH_TIME, delta)
	self._look_target = lr[0]
	self._look_vel = lr[1]
...
func _tick_fov(delta: float) -> void:
	...
	target += FOV_DRIFT_ADD * self._drift_blend
	var rate := FOV_ATTACK_RATE if target > self.fov else FOV_RELEASE_RATE
	self.fov = lerpf(self.fov, target, _approach(rate, delta))
