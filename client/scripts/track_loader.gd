...
static var _road_tex: ImageTexture = null
...
static func _make_road(parent: Node3D, prim: Dictionary, default_color: Color, override_mat: StandardMaterial3D) -> void:
	...
	var mesh := _build_road_mesh(samples, walls, wall_h)
	var mi := MeshInstance3D.new()
	mi.mesh = mesh
	var mat := override_mat
	if mat == null:
		mat = StandardMaterial3D.new()
		mat.albedo_color = color
		mat.albedo_texture = _road_texture()  # repeating asphalt + markings (tints by color)
		mat.roughness = 0.8
		mat.metallic = 0.1
	...
	# Collision: the surface as a smooth trimesh (built without walls), plus each
	# side wall as a chain of SOLID convex prisms — volume so a fast car can't
	# tunnel through (no CCD needed), shared edges so there's no step to catch on.
	var surf := CollisionShape3D.new()
	surf.shape = _build_road_mesh(samples).create_trimesh_shape()
	body.add_child(surf)
	if walls:
		_add_road_wall_colliders(body, samples, wall_h)
...
static func _sample_road(prim: Dictionary) -> Array:
	...
	if nodes.size() < 2:
		return []
	var size := _vec3_from_array(prim.get("size", []))
	var seg: int = int(prim.get("segments", ROAD_DEFAULT_SEGMENTS_PER_SPAN))
	if seg < 1:
		seg = 1
...
static func _road_texture() -> ImageTexture:
	if _road_tex != null:
		return _road_tex
	var s := 64
	var half := 32   # s / 2
	var quarter := 16  # s / 4 (dash = half the tile)
	var img := Image.create(s, s, false, Image.FORMAT_RGB8)
	for y in range(s):
		for x in range(s):
			var c := 0.70  # asphalt body (darkens the base colour a touch)
			if x < 2 or x >= s - 2:
				c = 1.0    # solid edge lines
			elif absi(x - half) <= 1 and (y % half) < quarter:
				c = 1.0    # dashed centre line
