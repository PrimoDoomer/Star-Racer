...
static func _make_curve(parent: Node3D, prim: Dictionary, default_color: Color, override_mat: StandardMaterial3D) -> void:
	...
	if override_mat != null:
		...
		mat.roughness = 0.8
		mat.metallic = 0.1
		# Double-sided: the ramp is a closed solid, but this guards against any face
		# being culled from a grazing angle.
...
# A SOLID curved ramp: the drive surface plus a flat ground bottom, a vertical back
# cap and two curved side walls — so it has real thickness and reads from any angle
# (the old single-surface mesh was invisible from the front, only its back showed).
static func _build_curve_mesh(width: float, height: float, length: float, segments: int) -> ArrayMesh:
	...
	# Surface profile (z, y) sampled along the quarter-circle, shared by every face.
	...
	# --- Flat bottom on the ground plane (y = 0) ---
	var bot := verts.size()
	...
	# --- Vertical back cap (z = length, the high end) ---
	var back := verts.size()
	...
	# --- Side walls (x = ±hw): strip between the curve and its ground projection ---
