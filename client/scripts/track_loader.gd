...
static func build(parent: Node3D, track_def: Dictionary) -> Dictionary:
	var floor_mat: StandardMaterial3D = load("res://tracks/aurora_circuit/floor_mat.tres")
	...
	for prim in primitives:
		...
		match kind:
			...
			"arc":
				_make_arc(parent, prim, FLOOR_DEFAULT_COLOR, floor_mat)
			"decor":
...
static func _make_concrete_mat() -> StandardMaterial3D:
	...
	mat.albedo_texture = load("res://tracks/aurora_circuit/wall_metal_color.png")
	mat.normal_enabled = true
	mat.normal_texture = load("res://tracks/aurora_circuit/wall_metal_normal.png")
	mat.roughness_texture = load("res://tracks/aurora_circuit/wall_metal_roughness.png")
	mat.roughness = 1.0
	mat.metallic = 0.55
