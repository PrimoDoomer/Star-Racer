...
/// The driving surface is tessellated this many times finer than the authored
/// `segments` (which drive the walls). Small facets — well under the ~4.8-unit
/// car — let the box ride the curvature smoothly instead of catching on edges.
/// Cheap at runtime: the surface is one BVH-backed trimesh, so the per-tick cost
/// scales with the few (wide) triangles under the car, not the total. Build cost
/// is one-time. Mirrors client.
const ROAD_SURFACE_REFINE: u32 = 8;
/// Side walls are tessellated this many times finer than the authored `segments`.
/// A convex prism can't follow a CONCAVE edge, so on the inside of a turn coarse
/// prisms "cut the corner" and intrude into the racing line; finer prisms shrink
/// that intrusion to nothing while keeping the collider count bounded. Mirrors client.
const ROAD_WALL_REFINE: u32 = 3;
...
impl TrackDef {
    ...
    /// World-space height of the road driving surface nearest the given (x, z) —
    /// the road sample closest in the horizontal plane. Used to seat the starting
    /// grid ON an elevated/curved road instead of a flat plane at the gate height
    /// (otherwise cars spawn floating and tumble onto the track at GO). `None` when
    /// the track has no road, or the nearest sample is implausibly far (off-road).
    pub fn road_surface_y_at(&self, x: f64, z: f64) -> Option<f64> {
        let mut best: Option<(f64, f64)> = None; // (horizontal dist², surface y)
        for prim in &self.primitives {
            ...
            for s in road_samples(prim) {
                ...
                let d2 = (w.x - x).powi(2) + (w.z - z).powi(2);
                if best.is_none_or(|(bd, _)| d2 < bd) {
                    best = Some((d2, w.y));
                }
            }
        }
        // Guard against snapping to a road far across the room.
        best.filter(|&(d2, _)| d2 < 60.0 * 60.0).map(|(_, y)| y)
    }
...
/// The authored segments-per-span for a road (clamped to ≥ 1).
fn road_segments(prim: &Primitive) -> u32 {
    prim.segments
        .unwrap_or(ROAD_DEFAULT_SEGMENTS_PER_SPAN)
        .max(1)
}

/// Road samples at the authored density — used for the side walls, the bots'
/// centerline, and the visual ribbon. The driving surface is sampled FINER (see
/// `ROAD_SURFACE_REFINE` in `build_road_colliders`) so the car rides smoothly.
fn road_samples(prim: &Primitive) -> Vec<RoadSample> {
    sample_road(prim, road_segments(prim))
}

/// Sample a road's Catmull-Rom centerline into `seg` evenly-stepped points per
/// span, carrying the per-point width and bank (linearly interpolated between
/// nodes), passing exactly through each control node. Client `_sample_road`
/// mirrors this — keep the two in sync.
fn sample_road(prim: &Primitive, seg: u32) -> Vec<RoadSample> {
...
fn build_road_colliders(prim: &Primitive, collider_set: &mut ColliderSet) {
    ...
    // Surface: a FINE trimesh (one cheap collider) so small facets ride smoothly.
    let fine = sample_road(prim, road_segments(prim) * ROAD_SURFACE_REFINE);
    let (verts, indices) = road_surface_trimesh(&road_frames(&fine));
    if !indices.is_empty() {
    ...
    // Walls: solid convex prisms, finer than the surface samples so they don't cut
    // the corner into the racing line on the inside of a turn (bounded count).
    if walls {
        let frames = road_frames(&sample_road(prim, road_segments(prim) * ROAD_WALL_REFINE));
...
mod tests {
    ...
    fn road_builds_surface_trimesh_plus_wall_prisms() {
        use rapier3d_f64::prelude::ColliderSet;
        // 3 nodes, segments = 4 → 8 coarse spans. Surface = 1 trimesh; walls are
        // ROAD_WALL_REFINE× finer → (8·refine) spans × 2 prisms.
        let track = road_track(r#", "segments": 4, "walls": true"#);
        let mut cs = ColliderSet::new();
        track.build_colliders(&mut cs);
        assert_eq!(cs.len(), 1 + (8 * ROAD_WALL_REFINE as usize) * 2);
    }
