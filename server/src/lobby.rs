...
impl PhysicsWorld {
    ...
    fn insert_body(&mut self, pos: Vec3Proto) -> RigidBodyHandle {
        ...
        let collider = ColliderBuilder::cuboid(1.3, 0.6, 2.4)
            .mass_properties(MassProperties::new(
                ...
                Vec3::new(0.0, COM_Y, 0.0),
                MASS,
                Vec3::new(INERTIA_X, INERTIA_Y, INERTIA_Z),
            ))
            .friction(0.0)
    ...
    /// Average upward surface normal under the car, from its solid contact
    /// manifolds (each oriented to point up, then filtered to near-vertical so
    /// wall touches are ignored). `None` when airborne or only touching walls.
    /// The client mirrors this with its downward ground-ray's collision normal.
    fn ground_normal(&self, handle: RigidBodyHandle) -> Option<Vec3> {
        let rb = self.rigid_body_set.get(handle)?;
        let &collider = rb.colliders().first()?;
...
impl Lobby {
    ...
    fn process_player_events(&mut self, delta: f64) {
        ...
        for (nickname, racer) in &mut self.racers {
            ...
            // Surface normal under the car (for the surface-aligned uprighting
            // below) — read before the mutable borrow of the body.
            let ground_n = self.physics.ground_normal(racer.rigid_body);
            ...
            if grounded {
                ...
                let target = match ground_n {
                    Some(n) if n.y > GROUND_ALIGN_MIN_NY => n,
                    _ => world_up,
                };
